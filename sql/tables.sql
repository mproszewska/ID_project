...
CREATE TABLE injuries ( 
	...
	duration             interval  ,
	"date"		     timestamp,
	CONSTRAINT uq_injuries_accident_id_users_id UNIQUE ( user_id,accident_id,"date" ),
...
ALTER TABLE sections ADD CONSTRAINT cns_sections CHECK ( coalesce(min_age,0) >= 0 and (coalesce(min_age,0) <= coalesce(max_age,2147483647) ));

ALTER TABLE sections ADD CONSTRAINT cns_sections_0 CHECK ( coalesce (min_members,0)>= 0 and ( coalesce (min_members,0) <=  coalesce (max_members,2147483647) ));
